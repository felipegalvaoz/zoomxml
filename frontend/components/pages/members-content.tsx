...
export function MembersContent({ companyId }: MembersContentProps = {}) {
  ...
  const fetchMembers = async (page = 1, search = "") => {
    ...
    try {
      ...
      const response = await backendApiCall<Member[]>(`/api/companies/${companyId}/members?${params}`)
      setMembers(response || [])

      // Buscar informações da empresa
      const companyResponse = await backendApiCall<Company>(`/api/companies/${companyId}`)
